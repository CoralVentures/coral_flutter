...
class DevtoolsDb {
  ...
  DevtoolsDb copyWith({
    ...
    bool clearBottomNavState = false,
    // CORAL_CLI_COPY_WITH_PARAMETER
  }) {
    var _bottomNavState = bottomNavState ?? this.bottomNavState;
    if (clearBottomNavState) {
      _bottomNavState = null;
    }
    // CORAL_CLI_COPY_WITH_CLEAR

    return DevtoolsDb(
      bottomNavState: _bottomNavState,
...
void remoteReduxDevtoolsOnEvent({
  ...
  if (shouldConnectReduxRemoteDevtools) {
    ...
    switch (blocTypeEnum) {
      case BlocType.bottomNav:
        devtoolsDB = devtoolsDB.copyWith(
          bottomNavState: state as BottomNavState,
        );
        break;
      // CORAL_CLI_ON_EVENT
    }
...
void remoteReduxDevtoolsOnClose({
  ...
  if (shouldConnectReduxRemoteDevtools) {
    ...
    switch (blocTypeEnum) {
      case BlocType.bottomNav:
        devtoolsDB = devtoolsDB.copyWith(clearBottomNavState: true);
        break;
      // CORAL_CLI_ON_CLOSE
    }
  }
